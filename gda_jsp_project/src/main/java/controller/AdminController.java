...
public class AdminController extends MskimRequestMapping {
	...
	public String instructorlist(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    int pageNum = 1;
	    if (request.getParameter("pageNum") != null) {
	        pageNum = Integer.parseInt(request.getParameter("pageNum"));
	    }
	    ...
	    if (keyword != null && !keyword.trim().isEmpty()) {
	        instructorList = adminDAO.searchInstructorsByName(keyword); // 이름 + 강사
	        request.setAttribute("keyword", keyword);
	    } else {
	        instructorList = adminDAO.instructorList(); // 강사만 전체 출력
	    }

	    PageInfo<UserDTO> pageInfo = new PageInfo<>(instructorList);
	    request.setAttribute("pageInfo", pageInfo);
	    request.setAttribute("userList", instructorList); // 재사용
