...
public interface InquiryMapper {
	...
	@Select("SELECT i.*, l.title AS lectureTitle, u.name AS userName " + "FROM inquiries i "
			+ "LEFT JOIN lectures l ON i.lecture_id = l.lecture_id " + "LEFT JOIN users u ON i.user_id = u.user_id "
			+ "WHERE i.inquiry_id = #{inquiryId}")
	@Results({ @Result(property = "inquiryId", column = "inquiry_id"), @Result(property = "userId", column = "user_id"),
			@Result(property = "lectureId", column = "lecture_id"), @Result(property = "title", column = "title"),
